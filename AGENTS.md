...
Four services (peer directories) plus `docs/`:

| Directory | Stack | Status |
|---|---|---|
| `commerce-admin/` | React 19 + Vite 6 + Ant Design 5 | Implemented (base CMS with mock API) |
| `commerce-core-service/` | Spring Boot Java | Not scaffolded (owned by Java engineer) |
...
Core Java implementation details are intentionally left for the Java engineer. The `docs/` folder defines service responsibilities, API contracts, and database models.
...
- **commerce-core-service**: Owns `identity`, `marketplace`, `ingestion`, `analytics` schemas. Provides auth, CRUD for all marketplace entities, source/import flow, and analytics views.
...
## Database Schemas (planned, not implemented)

Five logical schemas: `identity` (users, permissions), `marketplace` (sellers, buyers, categories, products, variants, images, reviews, orders), `ingestion` (data_sources, sync_runs, raw_snapshots, source_products, source_reviews), `ai` (embeddings, analysis, chat sessions, query logs), `analytics` (views for text-to-SQL).
...
- `commerce-core-service-nestjs/` is a NestJS replacement for the Java core service. It currently lives in-repo as untracked files.
